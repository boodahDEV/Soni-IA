...
class Interactive(abc.ABC):
        ...
        while self._sim_time < self._current_time:
            ...
            if not self._sync or act is not None:
               ...
               # obs.shape 
                var1 = []
                var1 = act
                actions = [int(var2) for var2 in var1]
                print("Action ", actions, "Reward ", rew)
                ...
                if done:
                    ...
                    self._episode_returns = 0
                    self._prev_episode_returns = 0
            return done, rew, actions, _info, obs.shape
    ...
    def run(self):
        ...
        archivo = open("Controlador.csv", "w")
        ...
        while True:
            ...
            now = time.time()
            _info = self._update(now - prev_frame_time)

            prev_frame_time = now
            self._draw()
            self._win.flip()
            archivo.write(str(_info))
            archivo.write(";")
            archivo.write("\n")
            #archivo.write(str(self._update(now - prev_frame_time)))
             #archivo.write(";")
            #archivo.write(str(done))
            #archivo.write(";")
            #archivo.write(str(obs.shape))
            #archivo.write(";")
